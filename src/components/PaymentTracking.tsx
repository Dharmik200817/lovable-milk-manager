...
export const PaymentTracking = () => {
  ...
  const loadCustomers = async () => {
    try {
      ...
      console.log('Loaded customers:', customersWithBalances);
              ...
              {formData.customerName && (
                <div className="p-3 bg-blue-50 rounded-lg">
                  <p className="text-sm font-medium text-blue-900">
                    Current Pending Amount: ₹{Math.ceil(getCustomerPendingAmount(formData.customerName))}
                  </p>
                </div>
              )}
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {isLoading ? (
                ...
                filteredPayments.map((payment) => {
                  const remainingBalance = getCustomerPendingAmount(payment.customer_name);
                  return (
                    <tr key={payment.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        {remainingBalance > 0 ? (
                          <span className="text-red-600">₹{Math.ceil(remainingBalance)}</span>
                        ) : (
